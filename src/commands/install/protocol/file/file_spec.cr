...
describe Zap::Commands::Install::Protocol::File, tags: "protocol" do
...

  {
    ...
    it "should normalize specifiers (#{specifier_data[:specifier]})" do
      ...
      path_info = Zap::Commands::Install::Protocol::PathInfo.from_str(specifier, base_directory)
      if path_info
        path_info.file = specifier_data[:file]
        path_info.dir = specifier_data[:dir]
      end

      Zap::Commands::Install::Protocol::File.normalize?(specifier, path_info).should eq(expected)
    end
  end
end
