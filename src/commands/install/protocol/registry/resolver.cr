...
struct Zap::Commands::Install::Protocol::Registry::Resolver < Zap::Commands::Install::Protocol::Resolver
  ...
  def valid?(metadata : Package) : Bool
    ...
    metadata.kind.registry? && (
      (range_set.is_a?(String) && range_set == metadata.version) ||
        (range_set.is_a?(Utils::Semver::Range) && range_set.satisfies?(metadata.version))
    )
  end
  ...
  private def fetch_metadata(*, pinned_version : String? = nil) : Package?
    ...
    state.store.with_lock("#{@base_url.to_s}/#{@package_name}", state.config) do
      ...
      raw_metadata = manifest.get_raw_metadata?(pinned_version ? Utils::Semver.parse(pinned_version) : self.specifier)
      unless raw_metadata
