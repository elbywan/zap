module Zap::Installer::Isolated::Helpers::Registry
  def self.install(dependency : Package, install_path : Path, *, installer : Zap::Installer::Base, state : Commands::Install::State)
    ...
    installed = begin
      Backend.install(dependency: dependency, target: install_path, store: state.store, backend: state.install_config.file_backend) {
