...
class Data::Package
  struct PackageExtension
    ...
    getter dependencies : Hash(String, String | Alias?)? = nil
    @[JSON::Field(key: "optionalDependencies")]
    getter optional_dependencies : Hash(String, String | Alias?)? = nil
    ...
    def merge_into(pkg : Data::Package)
      dependencies.try &.each do |name, version|
        if version.nil?
          pkg.dependencies.try &.delete(name)
        else
          hash = (pkg.dependencies ||= Hash(String, String | Alias).new)
          hash[name] = version
        end
      end
      optional_dependencies.try &.each do |name, version|
        if version.nil?
          ...
          hash = (pkg.optional_dependencies ||= Hash(String, String | Alias).new)
          hash[name] = version
        end
      end
