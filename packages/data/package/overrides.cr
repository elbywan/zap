...
class Data::Package
  struct Overrides
    ...
    record Override,
      ...
      def matches_package?(metadata : Data::Package) : Bool
        (
          version == "*" ||
            specifier == "*" ||
    ...
    def override?(metadata : Data::Package, ancestors : Iterable(Package | Lockfile::Root), *, match_ancestors : Bool = true) : Override?
    ...
    protected def read_object(pull : JSON::PullParser, *, parents : Array(Parent)? = nil, current_raw : Hash(String, JSON::Any) = @inner_raw)
      ...
      loop do
        ...
        name, version = Utils::Misc.parse_key(key)
