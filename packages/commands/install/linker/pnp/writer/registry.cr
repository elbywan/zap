module Commands::Install::Linker::PnP::Writer::Registry
  def self.install(dependency : Data::Package, install_path : Path, *, linker : Linker::Base, state : Commands::Install::State)
    installed = begin
      Backend.link(dependency: dependency, target: install_path, store: state.store, backend: state.config.file_backend, pipeline: state.pipeline) {
      ...
      Backend.link(backend: :copy, dependency: dependency, target: install_path, store: state.store, pipeline: state.pipeline) { }
    end
