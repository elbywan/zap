...
class Commands::Install::CLI < Commands::CLI
  ...
  def on_install(
    ...
    *,
    ...
    Helpers.flag("--reporter <plain|interactive|null>", "Force the output reporter instead of auto-detecting it from the terminal. #{"[env: ZAP_INSTALL_REPORTER]".colorize.dim}") do |reporter|
