...
class Commands::Install::Installer::Isolated < Commands::Install::Installer::Base
  ...
  def initialize(
    ...
    *,
    ...
    @hoist_patterns = hoist_patterns.map &->Utils::Misc.parse_pattern(String)
    @public_hoist_patterns = public_hoist_patterns.map &->Utils::Misc.parse_pattern(String)
  end
  ...
  private def install_package(
    package : Data::Package | Data::Lockfile::Root,
    *,
    ancestors : Ancestors,
    root_path : Path? = nil,
    optional : Bool = false
    ...
    if resolved_peers
      ...
      Log.debug { "(#{package.is_a?(Data::Package) ? package.key : package.name}) Linking #{dependency.key}: #{source} -> #{target}" }
  ...
  private def hoist_package(package : Data::Package, install_folder : Path)
