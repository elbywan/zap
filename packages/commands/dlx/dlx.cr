...
module Commands::Dlx
  ...
  def self.run(
    ...
    main_package = Data::Package.init?(Path.new(config.prefix))
    ...
    if dlx_config.command.empty?
      pkg_json = Data::Package.init(path / "node_modules" / packages[0][0])
      if bin = pkg_json.bin
  ...
  ) : Array({String, String | Data::Package::Alias})
    config_packages.map do |package|
      name, version = Utils::Misc.parse_key(package)
  ...
  def self.get_identifier_and_path(packages : Array({String, String | Data::Package::Alias}), *, prefix : String? = nil)
