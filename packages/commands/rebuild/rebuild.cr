module Commands::Rebuild
  def self.run(
    ...
    scripts = [] of Data::Package::Scripts::ScriptData
    ...
    scope.each do |workspace_or_main_package|
      ...
      self.crawl_native_packages(root_path.to_s) do |module_path|
        pkg = Data::Package.init?(Path.new(module_path))
        next unless pkg
        filters = rebuild_config.packages
        if filters && filters.size > 0
          matches = filters.any? do |filter|
            name, semver = Utils::Misc.parse_key(filter)
            pkg.name == name && (!semver || Semver.parse(semver).satisfies?(pkg.version))
          end
          next unless matches
        end
        pkg_scripts = pkg.scripts || Data::Package::LifecycleScripts.new

        install_args = rebuild_config.flags.try { |flags| " #{flags.join(" ")}" } || ""
        preinstall_script = pkg_scripts.preinstall.try do |preinstall_script|
          Data::Package::Scripts::ScriptDataNested.new(pkg, module_path, :preinstall, preinstall_script)
        end
        postinstall_script = pkg_scripts.postinstall.try do |postinstall_script|
          Data::Package::Scripts::ScriptDataNested.new(pkg, module_path, :preinstall, postinstall_script)
        end
        script_data = Data::Package::Scripts::ScriptData.new(
          pkg,
          module_path,
          "install",
