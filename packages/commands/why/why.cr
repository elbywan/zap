module Commands::Why
  ...
  def self.run(config : Core::Config, why_config : Why::Config)
    ...
    results = Hash(Data::Package, Array(PackageResult)).new
    ...
    output_by_package = results.map do |package, results|
      output = String.build do |str|
        str << "#{package.key}".colorize.yellow.bold.underline << Shared::Constants::NEW_LINE
  ...
  private def self.format_root_results(root, root_results, *, config : Why::Config)
    String.build do |str|
      str << root_results
        ...
        .join(Shared::Constants::NEW_LINE)
      str << Shared::Constants::NEW_LINE
    end
  end
  ...
  private def self.direct_ancestor_output(direct_ancestor : String, direct_ancestor_results : Array(PackageResult), *, config : Why::Config)
    String.build do |result_str|
      direct_ancestor_name, direct_ancestor_version = Utils::Misc.parse_key(direct_ancestor)
      result_str << "#{direct_ancestor_name.colorize.bold.cyan}#{"@#{direct_ancestor_version}"}"
      unless config.short
        result_str << Shared::Constants::NEW_LINE
  ...
  private def self.ancestor_path_output(result : PackageResult, *, last_ancestor : Bool = false)
    ...
    ancestors_str = result[:ancestors].reverse!.map_with_index { |(ancestor, type), index|
      name, version = Utils::Misc.parse_key(ancestor.key)
