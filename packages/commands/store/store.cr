module Commands::Store
  def self.run(config : Core::Config, store_config : Store::Config)
  ...
  def self.clear(config : Core::Config, store_config : Store::Config)
  ...
  def self.clear_packages(config : Core::Config, store_config : Store::Config)
    packages_path = Path.new(config.store_path) / ::Store::PACKAGES_STORE_PREFIX
    puts "💣 Nuking packages at '#{packages_path}'…"
    FileUtils.rm_rf(packages_path)
    puts "💥 Done!"
  end

  def self.print_path(config : Core::Config, exec_config : Store::Config)
