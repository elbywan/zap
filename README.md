...
Patches are applied to the linked node_modules copy after install; the store keeps the pristine package. Patch files are plain git-style unified diffs, so they can also be hand-written or produced by `git diff` and registered under `zap.patched_dependencies` in package.json. The key matches the exact version, any range, or the bare package name:
